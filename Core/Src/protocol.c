...
#include "usart.h"           // 协议层调用驱动层接口
#include "RingFrameQueue.h"  // 协议层使用队列
...
{
    uint8_t txBuffer[PROTOCOL_FRAME_SIZE_FEEDBACK];
    uint16_t frameLen;
    HAL_StatusTypeDef status;
    
    if (pFeedbackFrame == NULL)
    {
        return false;
    }
    
    // 1. 协议层：组帧
    frameLen = Protocol_PackFeedbackFrame(pFeedbackFrame, txBuffer);
    if (frameLen == 0)
    {
        return false;  // 组帧失败
    }
    ...
    // 2. 协议层调用驱动层：发送数据
    status = USART2_SendData(txBuffer, frameLen, timeout);
...
bool Protocol_ProcessReceivedFrame(Protocol_ControlFrame_t *pControlFrame)
{
    ...
    // 1. 协议层调用驱动层：从队列中取出数据
    ret = RFQ_Pop(&UART2_RX_RFQ, &frame);
    if (ret != 0)
    {
        return false;  // 队列为空
    }
    ...
    // 2. 协议层：解帧
    return Protocol_UnpackControlFrame(frame.data, frame.len, pControlFrame);
}
